...
export class Apellido {
  constructor(
    private readonly _primerApellido: string,
    private readonly _segundoApellido: string,
  ) {
  ...
  equals(otro: Apellido) {
    return (
      this._primerApellido == otro.primerApellido &&
      this._segundoApellido == otro.segundoApellido
    );
  }
  ...
  protected validate(primerApellido: string, segundoApellido: string): void {
    ...
    if (primerApellido.length > 30) {
      throw new ArgumentOutOfRangeException(
        'primerApellido esta fuera de rango',
      );
    }
    if (segundoApellido.length > 30) {
      throw new ArgumentOutOfRangeException(
        'segundoApellido esta fuera de rango',
      );
    }
  }
}
