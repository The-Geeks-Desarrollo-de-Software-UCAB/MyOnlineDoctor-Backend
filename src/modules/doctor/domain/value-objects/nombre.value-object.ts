...
export class Nombre {
  constructor(
    private readonly _primerNombre: string,
    private readonly _segundoNombre: string,
  ) {
  ...
  equals(otro: Nombre) {
    return (
      this._primerNombre == otro.primerNombre &&
      this._segundoNombre == otro.segundoNombre
    );
  }
  ...
  protected validate(primerNombre: string, segundoNombre: string): void {
    ...
    if (!(segundoNombre == null) && segundoNombre.length > 30) {
      throw new ArgumentOutOfRangeException(
        'segundoNombre esta fuera de rango',
      );
    }
  }
}
