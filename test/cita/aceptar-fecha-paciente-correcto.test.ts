...
describe('Un paciente acepta una cita que se le agendó', () => {

    it('deberia retornar la cita aceptada', async () => {
        ...
        //Act
        const action = async () => {
            await servicio.execute(doctor.id.id, paciente.idPaciente.id);
        }
